...
describe("geminiInteractionsToCompletionRequest", () => {
  ...
  it("handles Turn with empty parts array — user/assistant produce empty-content message", () => {
    const result = geminiInteractionsToCompletionRequest({
      model: "gemini-2.5-flash",
      input: [
        { role: "user", parts: [] },
  ...
  it("handles Turn with empty parts array — non-user/non-assistant role is skipped", () => {
    const result = geminiInteractionsToCompletionRequest({
      model: "gemini-2.5-flash",
      input: [
        { role: "system", parts: [] },
    ...
    // The system turn with empty parts is skipped; only the user turn is kept
